...
5. Detail Player integrates detail/watch serving against the published catalog backend
6. Reviewer validates published-only serving, route integration discipline, public/runtime correctness, and ownership boundaries before merge
...
- Reconnect public browse/search/detail/watch reads to published backend data only
- Owned surfaces:
...
Acceptance criteria:
- Public browse/search/detail/watch routes resolve published catalog data from backend read boundaries rather than withdrawn seed-only runtime data
...
- Search/filter inputs and detail/watch selection logic consume published catalog contracts, not route-local seed shortcuts
...
- The repo still lacks published-catalog read serving for public browse/search/detail/watch routes
