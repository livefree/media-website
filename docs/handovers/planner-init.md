...
- Published reference-informed expectations for browsing routes from `web-to-colon/main.html` and the detail route from `web-to-colon/page.html`
...
- UI Shell should wait for the shared catalog shape, then align `/`, `/movie`, `/series`, and `/anime` to the compact nav, search, filter, dense-grid, and pagination structure from `web-to-colon/main.html`
...
- Detail Player should plan around the `web-to-colon/page.html` structure: metadata header, synopsis, source tabs, optional episode selector, player shell, and download resources
